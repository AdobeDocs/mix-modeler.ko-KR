...
>title="요인 기여도 분류"
>abstract="요인 기여도 분류는 모델에 포함된 다양한 요인에 기인할 수 있는 기본 전환의 비율을 보여 줍니다.<br/><br/>순수 기본은 모델에 포함된 마케팅 접점 및 요인과 독립적으로 발생하는 근본적인 전환을 나타냅니다. 여기에는 브랜드 에쿼티, 반복 구매, 유기적 수요, 장기 시장 트렌드 및 계절성에 따른 전환이 포함됩니다."
...
>title="접점 위치별 분류"
>abstract="이 시각화는 모든 전환 경로에서 접점 및 접점 위치에 따른 속성 전환의 분류를 보여줍니다. 시각화는 접점이 나머지 위치와 다른 위치의 다른 접점보다 더 나은 위치에서 기여하는지 비교합니다."
...
>title="총 볼륨"
>abstract="총 볼륨은 사용자가 접점을 터치한 총 횟수입니다. 또한 전환되지 않는 경로뿐만 아니라 달성하는 경로에 나타나는 접점도 포함됩니다."


>[!CONTEXTUALHELP]
>id="models_diagnostics_modeldateinfo"
>title="모델 날짜 기준"
>abstract="이 테이블의 데이터는 특정 기간 동안만 생성됩니다.  **[!UICONTROL As of]** 날짜는 데이터가 생성된 시간을 나타내며 지정된 날짜부터 1년 전까지 데이터를 기반으로 합니다."
